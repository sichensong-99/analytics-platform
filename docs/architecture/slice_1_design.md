...
| Reviewers | Self (project owner). Will share with stakeholder after Day 5 demo. |
| Supersedes | None (first slice design doc) |
| Related Docs | `docs/data_modeling/star_schema_ddl.sql` (v1.1), `docs/existing_data_inventory.md` §5.1, `docs/legacy_panoply_etl.md` (v3),  Decision Log §10–16 |
...
| Day 5 EOD | **stakeholder Demo** | Live walkthrough of the page on the new platform |
...
3. **stakeholder visibility**: Marketing and merchandising use this page weekly. A working demo creates immediate stakeholder buy-in for the platform direction.
...
|---|---|---|
| stakeholder | Project sponsor (cost reduction + platform direction) | Day 5 demo recipient; reconciliation diff < 2% is the trust threshold |
...
│  analytics_catalog.shopify_raw    federated_catalog    │
...
| Property | Value |
|---|---|
| Catalog.schema | `analytics_catalog.shopify_raw` |
...
| Property | Value |
|---|---|
| Catalog.schema | `federated_catalog.triple_whale` (cosmetic catalog name — not actual Lakehouse Federation) |
...
| Attribution model used | `last_touch` (matches TW UI default and stakeholder's TW exposure) — see §8.3 for decision rationale |
...
| Property | Value |
|---|---|
| Catalog.schema.volume | `analytics_catalog.analytics_platform.raw_uploads` |
...
**Output**: `analytics_catalog.analytics_platform.dim_date`.
...
**Output**: `analytics_catalog.analytics_platform.dim_channel`.
...
**Input**: latest `ers_product_master_*.csv` in Volume + distinct SKUs from `shopify_raw.order_line` for slice 1 window.

**Output**: `analytics_catalog.analytics_platform.dim_product`.
...
**Output**: `analytics_catalog.analytics_platform.fact_orders_line`.
...
```
shopify_raw.order_line  (one row per SKU on an order)
       │
       │  inner join on order_id
       ▼
shopify_raw.order        (one row per order — for processed_at timestamp)
       │
...
**Rationale** (trade-off):
- ✅ Matches TW UI default — stakeholder and TW colleague see the same numbers in TW as in our portal, minimizing reconciliation friction during demo.
...
last_touch_per_order = (
    spark.table("federated_catalog.triple_whale.attribution_order_click")
...
This is the **trust-establishing** validation for stakeholder demo. Beyond automated DQ checks, run this comparison query at the close of Day 5:
...
WITH new_platform AS (
  ...
  FROM analytics_platform.fact_orders_line f
  JOIN analytics_platform.dim_date d ON f.date_key = d.date_key
  JOIN analytics_platform.dim_product p ON f.product_key = p.product_key
  JOIN analytics_platform.dim_channel c ON f.channel_key = c.channel_key
...
6. stakeholder demo: walkthrough of (a) the page, (b) reconciliation CSV, (c) "what comes next"
```
...
| R3 | Cross-year ISO week edge case (W52/W53) produces visible stakeholder-facing diff vs Panoply | Low | Medium | Document the difference proactively (`existing_data_inventory.md` §5.1.4 Insight #2 already covers); explain in demo as "intentional correction, not bug." |
...
| R5 | Reconciliation diff > 2% threshold | Medium | High (erodes stakeholder trust) | Drill into the diff using the §9.2 query output. Most likely cause: channel taxonomy mismatch — known and explainable. Pre-prepare a 3-bullet explanation for the demo. |
| R6 | Multi-tenant raw attribution table | `attribution_order_click` contains non-MERCHANT brands (TW pipeline does not filter by account). | Low (Slice 1) | Join on globally-unique `_triple_whale_order_id` isolates MERCHANT data — foreign-brand clicks cannot match MERCHANT orders. Performance cost only (oversized scan). Future: filter at ingestion by account_id. |

---

## 17. Open Questions (deferred to stakeholder alignment)

These do **not** block slice 1 implementation but should be confirmed with stakeholder during or right after Day 5 demo:

1. **Channel grouping mapping**: `legacy_channel_group` mapping (TW `google-ads` → "Paid Search", `meta` → "Paid Social", etc.) is Sia's best-effort. stakeholder confirmation on the full 15-row table avoids later relabeling friction.
2. **Non-attributed / Excluded display**: Currently displayed inline with other channels (Decision 14). stakeholder may prefer them visually de-emphasized (gray) — easy frontend tweak.
3. **Refund handling on quantity**: Slice 1 displays gross quantity (no refund deduction), matching legacy `Style_selling_df` behavior. Slice 3 (refunds) will introduce `is_refunded` flag and a "net quantity" toggle — stakeholder's preference informs slice 3 priority.
