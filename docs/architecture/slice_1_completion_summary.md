...
Migrate PBI `Style-channel (quantity)` page to the new platform end-to-end:
Shopify + Triple Whale → Databricks Kimball star schema → FastAPI metrics
service → Next.js portal, with quantitative reconciliation against the legacy
Panoply pipeline as the trust gate.
...
> See PROGRESS.md "Slice 1 量化基线" table.
...
## 5. Engineering Highlights (resume bullets)
...
| Item | Trigger | Owner |
|---|---|---|
| Replacement exclusion rerun | Fivetran syncs `order_metafield` | Sia |
| Reconciliation refresh post-replacement | After above | Sia |
| Leader demo + feedback | Demo script complete | Sia |
| Slice 2 (revenue page) | Demo feedback | Sia |
| Slice 2 backlog: order-grain fact | Slice 2 modeling | Sia |
...
This pattern caught two earlier blocking issues (wrong Shopify connector
pointing at Appaman, insufficient TW historical data) within hours.

### 7.2 Reconciliation as model correctness oracle
Reconciliation isn't just a trust-building artifact for Leader — it actively
...
upgrade trivial once Fivetran completes.

---

## 8. Resume Headlines (extracted from this slice)

> "Built end-to-end Kimball star schema on Databricks Lakehouse processing
> ~10M order lines, with multi-tier SLO data quality gates calibrated against
> empirical baseline, and quantitative reconciliation methodology against
> legacy BI achieving −1.51% diff with fully itemized residual attribution."

> "Designed materialized business rule column (`is_sales_attributable`)
> replacing scattered WHERE filters across legacy BI pipeline, with line-level
> refund netting model corrected via reconciliation-driven discovery."

> "Identified and corrected 3 legacy accuracy gaps: DST timezone drift
> (Decision 17), tag-based refund detection covering only 22% of native
> refunds, and cancelled-orders-counted-as-sales — all documented as
> intentional corrections in the migration."