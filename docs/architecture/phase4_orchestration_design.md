...
| Reviewers | Self; share with Databricks colleague + stakeholder after Slice 1 demo |
...
self-driving system, adding these production capabilities:
...
| stakeholder | Observer | Subscribes to digest alerts; sees migration confidence increase as DQ checks run daily |
...
This is Phase 4's signature engineering pattern.
...
```yaml
email_notifications:
  on_failure: [sia.song@example.com]
...
Why bother on success: gives stakeholder passive visibility ("the platform is
...
last_ingest = spark.sql(
    "SELECT MAX(_ingest_at) FROM analytics_platform.fact_orders_line"
).first()[0]
...
```sql
DESCRIBE HISTORY analytics_platform.fact_orders_line;
RESTORE TABLE analytics_platform.fact_orders_line TO VERSION AS OF <n>;
```
...
This is the answer to a common question: "Why didn't you use
...
This nuance is the heart of the answer — engineers who can articulate
