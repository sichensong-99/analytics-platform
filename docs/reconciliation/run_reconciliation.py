...
Purpose:
    ...
    legacy_channel_group) grain. Generate stakeholder-friendly Excel report with
...
Outputs:
    docs/reconciliation/reports/reconciliation_report.xlsx  (stakeholder-facing)
...
def write_excel(diff_df: pd.DataFrame, summary: dict, out_path: Path) -> None:
    """Write a multi-sheet, color-coded XLSX for stakeholder review."""
...
def _write_summary_sheet(ws, summary: dict) -> None:
    """Render the summary sheet — high-level stats stakeholder sees first."""
    bold = Font(bold=True)
