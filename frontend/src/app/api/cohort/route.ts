...
export async function GET() {
  if (!(await requireRole('admin', 'viewer'))) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  }
  const token = (await cookies()).get('auth-token')?.value;
  const upstreamUrl = `${METRICS_SERVICE_URL}/cohort`;
  try {
    const res = await fetch(upstreamUrl, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
    const text = await res.text();
    if (!text.trim()) return NextResponse.json({ error: 'empty', upstream_status: res.status }, { status: res.ok ? 502 : res.status });
    let data: unknown;
